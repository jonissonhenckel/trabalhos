...
void Listar(void) {
  ...
  while (opcao == 3) {
	...
	if (Turmaini > 1){
		for(inicial = 1; inicial <= Turmaini; inicial++){
            for(loop= 1; loop < Turmaini; loop++){
                    if(Dados[arrayAUX[loop]].Media > Dados[arrayAUX[loop+1]].Media){
                            AUX = arrayAUX[loop];
                            arrayAUX[loop] = arrayAUX[loop+1];
                            arrayAUX[loop+1] = AUX;
                    }
            }
    	}
    ...
    for (loop = 1; loop <= Turmaini; loop++) {
        ...
        printf("--------------------------------------\n");
        gotoxy(2, 3);
        printf("ID: %d\n", Dados[arrayAUX[loop]].Id);
        gotoxy(2, 5);
        printf("Matricula: %d\n", Dados[arrayAUX[loop]].Matricula);
        gotoxy(2, 7);
        printf("Nome: %s\n", Dados[arrayAUX[loop]].Nome);
        gotoxy(2, 9);
        printf("Nota1: %.2f\n", Dados[arrayAUX[loop]].Nota1);
        gotoxy(2, 11);
        printf("Nota2: %.2f\n", Dados[arrayAUX[loop]].Nota2);
        gotoxy(2, 13);
        printf("Media: %.2f\n", Dados[arrayAUX[loop]].Media);
        gotoxy(2, 15);
        printf("\t\t\tPaginas: %d / %d\n\n\n", contpage, Turmaini);
        contpage++;
        system("pause");
